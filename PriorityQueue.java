...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        //new Node is created with the given value and priority. It's set at the end of the heap
        Node newNode = new Node(e, priority, tree.size());
        //the new node will be added to the end of the tree list
        tree.add(newNode);
        //the new node is moved to its correct position in the heap
        pullUp(tree.size() -1);
        //the node is returned so it can be referenced later
        return newNode;
    }
    ...
    public boolean contains(E e) {
        //loops through all the nodes in the heap
        for (Node node : tree) {
            //will check if the node with a certain value exists in the heap and if it's still valid
            if (node.value.equals(e) && node.isValid()) {
                //returns true if the element is found
                return true;
            }
        }
        //will return left if the element is not found
        return false;
    }
